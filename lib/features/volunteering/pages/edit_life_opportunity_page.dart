...
class EditLifeOpportunityPage extends ConsumerStatefulWidget {
  final Opportunity opp;
  const EditLifeOpportunityPage({super.key, required this.opp});

  @override
  ConsumerState<EditLifeOpportunityPage> createState() => _EditLifeOpportunityPageState();
}

class _EditLifeOpportunityPageState extends ConsumerState<EditLifeOpportunityPage> {
  ...
  void initState() {
    super.initState();
    _titleController = TextEditingController(text: widget.opp.title);
    _locationController = TextEditingController(text: widget.opp.location);
    _descriptionController = TextEditingController(text: widget.opp.description);
    _eligibilityController = TextEditingController(text: widget.opp.eligibility);
    _contactMethodController = TextEditingController(text: widget.opp.contactMethod);
    _selectedCategory = widget.opp.category;
    _selectedDeadline = widget.opp.deadline;
  }
  ...
  Future<void> _updateOpportunity() async {
    ...
    final updatedOpp = Opportunity(
      ...
      postedBy: widget.opp.postedBy,
      organizationId: widget.opp.organizationId,
      status: widget.opp.status,
      createdAt: widget.opp.createdAt,
    );

    try {
      await ref.read(volunteerRepositoryProvider).saveLifeOpportunity(updatedOpp);
      if (mounted) {
                        ...
                        child: Form(
                          ...
                          child: Column(
                            ...
                            children: [
                              ...
                              ElevatedButton(
                                onPressed: _isSaving ? null : _updateOpportunity,
                                ...
                                child: _isSaving
                                    ...
                                    : const Text('Update Circular', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
                              ),
                            ],
                          ),
                        ),
